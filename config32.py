...
class miracl_compile:
    def compile_file(optim, file):
        ...
        if optim != 0:
            flags = " -mcpu=cortex-m4 -mthumb -mfloat-abi=softfp -mfpu=fpv4-sp-d16 -mgeneral-regs-only -std=c99 -O%d -c %s" % (optim, file)
        else:
            flags = " -mcpu=cortex-m4 -mthumb -mfloat-abi=softfp -mfpu=fpv4-sp-d16 -mgeneral-regs-only -std=c99 -c %s" % (file)
