 ...
 */
int get_random_blind(unsigned char *buf, u16 len)
{
	/* WARNING: this is a dummy implementation that should be replaced by
	 * a proper one in contexts where security is a concern.
	 */
	buf[0] = 1;
	for (u16 i = 1; i < len; i++) {
		buf[i] = 0;
...
int get_random_itoh(unsigned char *buf, u16 len)
{
	/* WARNING: this is a dummy implementation that should be replaced by
	 * a proper one in contexts where security is a concern.
	 */
